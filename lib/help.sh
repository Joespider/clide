...
CliHelp()
{
	...
	case ${option} in
		info)
			...
			echo -e "-shv, --shell-version\t\t\t: \"The Code Shell Version\""
		...
		function)
			case ${example} in
				...
				*)
					...
					echo -e "\t--compile-code-temp all\t\t\t\t: \"Compile ALL language new code template\""
					echo -e "\t--compile-code-temp <lang>\t\t\t: \"Compile a given language's new code template\""
...
gencodeCliHelp()
{
	...
	echo "To use this funtion, inside your source code, at the beginning of a line, put \"<<shell>>\" with the followed your choide of code tags"
	echo ""
	echo "<<shell>> if:true +-[help]:me +-el"
	echo "<<shell>> else +-[nothing]: +-el"
	echo ""
	echo -e "${cmd} <language> <code>"
	echo -e "${cmd} <code>"
